...
比赛推荐使用唤醒式连续语音。按键式单次录音 service 默认关闭，调试时需要显式开启：
...
`competition` 默认启用连续语音会话和 TTS 播报；实际参数为 `enable_voice:=true`、`enable_voice_session:=true`、`enable_capture_voice:=false`、`enable_tts:=true`、`audio_input_device:=plughw:CARD=Luna,DEV=0`、`audio_output_device:=default`。启动后在 UI 的“系统控制”页点击“一键启动比赛节点”，由总控台启动底盘/雷达、ZED、感知和导航；不需要再手动开其它终端。若现场需要临时静音：
...
任务书图片目录为 `/home/nvidia/ros2_ws/src/ylhb_llm/test_images`，目录内只保留一张 `.jpg/.jpeg/.png`：
...
UI 的“保存地图”默认保存到 `~/ros2_ws/src/maps/<map_name>.yaml/.pgm`；导航默认仍读取 `~/ros2_ws/src/my_map.yaml`。
