...
| `voice_input_node` | `ylhb_llm` | eMeet Luna/ALSA 按钮触发录音 | `/retail_ai/text_command` | 把语音指令转成文字指令；默认关闭，接上声卡后再开启 |
| `voice_output_node` | `ylhb_llm` | `/retail_ai/say_text` | `/retail_ai/voice_status` | 播报队列，支持优先级和打断；可只打印文本，也可启用 TTS 播放 |
| `retail_display_ui_node` | `ylhb_llm` | 任务按钮、系统状态、识别结果、购物车 | `/retail_ai/system_mode`, `/retail_ai/system_command`, `/retail_ai/text_command`, `/cmd_vel`, service `/retail_ai/start_b1_task` | 比赛现场显示屏、状态控制台和任务 D 驾驶舱 |
...
`CartState` 中的商品使用 `RecognizedProduct`，包含标准商品 ID、名称、类别、数量、单价、来源任务 ID。总价由本地代码按 `products.yaml` 计算，不让大模型直接决定最终金额。
...
| `request_timeout_sec` | `15.0` | 普通文本请求超时 | 文字解析慢时调大；比赛不建议超过 20 |
...
语音或文字控制走统一文字入口，ASR 结果也会发布到这个话题：
...
推荐使用 `plughw:CARD=Luna,DEV=0`，比 `hw:2,0` 更不依赖 card 编号。先只开 ASR，不开 TTS：
...
语音输入不是连续监听，而是按钮/服务触发。这样可以避免机器人播报被麦克风再次录进去。命令行触发一次录音识别：
...
```bash
ls -l /home/nvidia/task_images/task_b.png
```
