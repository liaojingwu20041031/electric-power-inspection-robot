...
> 迁移说明：本文档来自原 `ylhb-smart-retail-robot` 项目，当前仓库已切换为电力行业巡检机器人方向。底盘、建图、导航、ZED、语音和 Jetson 部署部分仍然有效；涉及“商品、货架、购物车、结算”的业务内容属于旧零售任务逻辑，后续会逐步替换为“站点、区域、路线、检查点、检测项、告警和巡检记录”。
...
2. 建图与定位
   RPLidar + 里程计 + TF -> SLAM Toolbox -> 保存地图 -> AMCL / Nav2 定位

3. 路线规划
   前端或配置文件创建站点、区域、路线和检查点，为每个检查点绑定检测项

4. 机器人巡航
   Nav2 按路线移动；路线中持续检测人员、未戴安全帽、障碍物、火源和烟雾

5. 检查点检测
   到点后播报“已到达指定位置，开始检查”，调整相机/云台，采集图像或视频

6. AI 识别与告警
   调用 YOLO/TensorRT、LocateAnything 或专项检测服务，识别开关状态、漏油、烟火、异物、表计/指示灯等

7. 后台展示与记录
   实时展示机器人状态、检查结果和告警，任务完成后生成巡检记录
```
...
navigation    启动 Nav2，默认优先读取 ~/ros2_ws/maps/my_map.yaml，缺失时回退到 ~/ros2_ws/src/my_map.yaml
...
`save_map` 默认保存到 `~/ros2_ws/src/maps/<map_name>.yaml/.pgm`；直接启动 `navigation.launch.py` 默认优先读取 `~/ros2_ws/maps/my_map.yaml`，缺失时回退到 `~/ros2_ws/src/my_map.yaml`。supervisor 的 `start_navigation` 默认同样使用 `~/ros2_ws/maps/my_map.yaml`；如果要让新地图成为默认导航地图，需要复制到推荐比赛地图路径，或启动导航时覆盖 `map:=...`。
...
│   └── my_map.pgm                       # 导航回退地图图像
├── maps/
│   ├── my_map.yaml                      # navigation.launch.py 优先读取的默认导航地图
│   └── my_map.pgm                       # navigation.launch.py 优先读取的默认导航地图图像
├── build/
├── install/
└── log/
```
...
当前仓库回退地图 `~/ros2_ws/src/my_map.yaml/.pgm` 已更新为实机保存结果，地图尺寸 `195 x 333`，分辨率 `0.05 m/pix`。直接启动 `navigation.launch.py` 时会优先使用 `~/ros2_ws/maps/my_map.yaml`，该文件不存在才回退到 `~/ros2_ws/src/my_map.yaml`。

注意：保存目录和导航默认地图是两个概念。比赛推荐导航地图是 `~/ros2_ws/maps/my_map.yaml`，`~/ros2_ws/src/my_map.yaml` 只作为兼容回退。如果要使用 UI 保存的新地图，需要复制成推荐默认地图，或启动导航时显式传 `map:=~/ros2_ws/src/maps/<map_name>.yaml`。
...
# 比赛默认从建图起点附近启动，AMCL 会先使用地图原点 (0, 0, 0) 初始化
...
如果 `~/ros2_ws/maps/my_map.yaml` 不存在，`navigation.launch.py` 会回退到：
...
```text
1. 建图起点和比赛启动点尽量一致。
2. 默认按建图原点初始化；如果机器人不在建图起点附近，先在 RViz/Foxglove 发布 /initialpose 覆盖。
3. 小车上电后先让 AMCL 稳定。
4. 如果定位不准，先原地缓慢旋转一圈，让雷达特征帮助粒子收敛。
5. Foxglove/RViz 发布 `/goal_pose` 时 `header.frame_id` 必须是 `map`。
6. 目标点不要贴墙，给局部规划器留出安全距离。
7. 若 /cmd_vel 有输出但车不动，优先检查底盘串口和 base_controller。
```

### 5.5 定位贴合调试

定位贴合问题先按几何关系排查，不要一开始就硬调 AMCL 参数：

```text
1. 先确认 bringup 已启动，/odom、/scan、/tf 都在发布；单独启动 navigation.launch.py 没有底层 odom，Nav2 会一直等 TF。
2. 机器人静止时，在 Foxglove/RViz 同时显示 /map、/scan、/particle_cloud。
3. 如果机器人不在建图原点附近，先发布 /initialpose；默认原点位姿只适合原点附近开局。
4. 再检查 /scan 与 /map 墙线是否像一个刚体一样整体错位。
5. 如果整体旋转错，优先校准 ylhb.urdf.xacro 里的 laser_link yaw。
6. 如果整体前后错，优先校准 laser_link x。
7. 如果整体左右错，优先校准 laser_link y。
8. 雷达外参基本正确后，再校准 wheel_radius 和 wheel_track，减少里程计累计误差。
9. 最后重新建图并用新图导航，避免拿旧地图硬调 AMCL。
```

判断顺序：

```text
/scan 静止时就和墙线对不上：优先查 laser_link 外参或地图质量。
/particle_cloud 在人工初始位姿附近不收敛：查初始位姿、雷达模型参数和地图是否匹配。
运动后才逐渐偏：优先查 wheel_radius、wheel_track、IMU/EKF 和底盘打滑。
```

`Starting point in lethal space` 排查顺序：

```text
1. 确认加载的是最新比赛地图，优先使用 ~/ros2_ws/maps/my_map.yaml。
2. 如果当前位置偏离建图原点，重新发布 /initialpose，让 /scan 与 /map 墙线贴合后再发目标点。
3. 在 Foxglove/RViz 叠加 /scan、/map、/particle_cloud，确认定位没有整体平移或旋转偏差。
4. 确认 global_costmap 只启用 static_layer 和 inflation_layer，没有误加 obstacle_layer。
5. 再检查 footprint 和 footprint_padding 是否过大，避免起点被车体外廓直接判为碰撞。
```

只有再次出现 TF future extrapolation 时，再考虑 EKF 的 `transform_time_offset` 和 `transform_timeout`。
...
| `default_navigation_map` | `/home/nvidia/ros2_ws/maps/my_map.yaml` | `start_navigation` 默认比赛地图 |
...
/map 正常发布
默认原点启动或已发布 /initialpose 覆盖，且 /scan 与 /map 基本贴合
AMCL 能定位
