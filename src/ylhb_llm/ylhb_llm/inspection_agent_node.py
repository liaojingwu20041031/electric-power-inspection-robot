...
def decide_local(request: Dict[str, Any], state: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if '急停' in normalized or '紧急停止' in normalized:
        return make_decision('emergency_stop', 'emergency_stop', text, '已发送急停。', safety_level='critical')
    if normalized in {'停止', '停下', '停下来', '刹车', '停'}:
    ...
    if any(word in normalized for word in ('开始巡逻', '开始巡检', '启动巡逻', '启动巡检', '执行巡逻', '执行巡检')):
        return make_decision('start_patrol', 'start_patrol_mode', text, '准备开始巡逻。')
    if '暂停' in normalized and ('巡逻' in normalized or '巡检' in normalized):
        return make_decision('pause_patrol', 'pause_patrol', text, '准备暂停巡逻。')
    if any(word in normalized for word in ('继续巡逻', '继续巡检', '恢复巡逻', '恢复巡检')):
        return make_decision('resume_patrol', 'resume_patrol', text, '准备继续巡逻。')
    if any(word in normalized for word in ('取消巡逻', '取消巡检', '终止巡逻', '终止巡检')):
        return make_decision('cancel_patrol', 'cancel_patrol', text, '准备取消巡逻。')
    for command in ('前进', '后退', '左转', '右转'):
        if command in normalized:
            return make_decision('motion', 'send_text_motion', text, f'执行{command}。', arguments={'command': command})
    if any(word in normalized for word in ('状态', '怎么样', '情况', '进度')):
        ...
        return make_decision('status_query', 'generate_local_status_reply', text, answer, answer, 'final')
    if any(word in normalized for word in ('你能做什么', '有什么功能', '功能', '怎么用')):
        answer = '我可以协助开始、暂停、继续、取消巡逻，执行短时运动，查询当前状态。'
        return make_decision('capability_query', 'generate_local_status_reply', text, answer, answer, 'final')
    return None
...
class InspectionAgentNode(Node):
    def __init__(self) -> None:
        ...
        self.create_subscription(String, self.get_parameter('text_command_topic').value, self.request_callback, 10)
    ...
    def request_callback(self, msg: String) -> None:
        ...
        try:
            ...
            if not policy.allowed:
                ...
                decision['speak'] = policy.reason or '当前指令已被安全策略拒绝。'
            else:
    ...
    def decide_llm(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.enable_llm_fallback or not self.qwen.available():
            ...
            return make_decision('llm_unavailable', 'generate_local_status_reply', text, answer, answer, 'final')
        prompt = (
            '只输出 AgentDecision JSON。不要 Markdown。'
