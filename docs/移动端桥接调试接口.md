...
Cloud Link 的 env、systemd、状态 Topic、SetBool 开关、日志和备份见 [Jetson 云平台连接运维](云平台连接运维.md)；协议字段见 [Robot Platform Protocol v1](protocol/robot-platform-v1.md)。
...
用途：把指定地图确认为 Nav2 和本地巡逻默认地图，并为新默认地图创建平台上传任务。当前默认地图再次调用时只上传，不重复归档。
...
默认地图切换成功后，即使快照或上传任务创建失败也不会回滚；此时 HTTP 仍返回成功，
`default_applied=true` 且 `upload.status=FAILED_TO_CREATE`。APP 只展示中文上传状态、
`map_asset_id` 和简短错误，不读取或上传 YAML/PGM，不保存设备 Token。
